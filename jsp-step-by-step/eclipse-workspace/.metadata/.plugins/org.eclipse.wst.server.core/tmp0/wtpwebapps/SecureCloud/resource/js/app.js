                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == null || data.length == 0) {
                            ...
                            g_cookie_expires = remember ? g_cookie_expires : null;

                            if (data.authorized) {
                                delCookie("loginUrl");
                                if (data.portal == "U") { // Users
                                    delCookie("loginname");
                                    delCookie("userId");
                                    delCookie("orgId");
                                    ...
                                    setCookie("loginname", loginname, g_cookie_expires);
                                    setCookie("userId", data.userId, g_cookie_expires);
                                    setCookie("orgId", data.orgId, g_cookie_expires);
                                    setCookie("organization", data.organization, g_cookie_expires);
                                    setCookie("systemName", data.systemName, g_cookie_expires);
                                    setCookie("roleId", data.roleId, g_cookie_expires);
                                    setCookie("portal", data.portal, g_cookie_expires);
                                    setCookie("OrgLogo", data.logo, g_cookie_expires);
                                    setCookie("token", data.token, g_cookie_expires);

                                    userInitHerf(data.orgId);
                                }
                                else { // Managers
                                    delCookie("loginname");
                                    delCookie("userId");
